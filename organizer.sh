...
remove_subject () {
    list_subjects
    echo "Please select the subject/project that you want to delete: "
    read subject
    if [ -d "$HOME/personal-organizer/$subject" ]; then
        echo "Are you sure you want to delete $subject and all of its contents? y or n"
        read confirmation
        if [ "$confirmation" == "y" ]; then
            rm -r "$HOME/personal-organizer/$subject"
...
while true; do
    ...
    echo "6. Delete a subject/project"
    echo "7. Exit"
    echo "------------------------"
